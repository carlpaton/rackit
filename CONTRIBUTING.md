...
The Ralph Loop stop hook requires `jq`. Install it once:

```bash
choco install jq
```

Restart your terminal after installing so `jq` is on your PATH.

---

## Required Plugins

Before running the Ralph Loop, install the plugin in Claude Code:

```
/plugin install ralph-loop@claude-plugins-official
```

Then run `/reload-plugins` to activate. This only needs to be done once per install scope (user or project). See `docs/ralph-loop.md` for full documentation.
...
Once the plugin is installed and the PRD is ready, run the Ralph Loop inside Claude Code:

```
/ralph-loop:ralph-loop "Work through PRD.json, implement each todo item in dependency order, commit after each, and mark done. Stop when all items are done." --max-iterations 20 --completion-promise "COMPLETE"
```
...
- **Supervised** — watch each iteration and cancel with `/ralph-loop:cancel-ralph` before the next task starts if something looks wrong. Recommended until you trust your PRD quality.
...
- `docs/ralph-loop.md` — how the Ralph Loop works, cost guidance, and when not to use it
